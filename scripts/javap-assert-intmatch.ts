...
function findClass(): string {
  ...
  const prefer = root + '/demo/int_match/classify_fn.class';
  ...
  while (stack.length) {
    ...
    for (const ent of fs.readdirSync(d)) {
      ...
      else if (st.isFile() && p.endsWith('classify_fn.class')) return p;
    }
  }
  throw new Error('classify_fn.class not found under ' + root);
}
...
function main(): void {
  ...
  if (!(out.includes('tableswitch') || out.includes('lookupswitch'))) {
    console.error('Assertion failed: expected tableswitch/lookupswitch in classify()');
    process.exit(1);
  }
  console.log('javap assertion passed: int match lowered to switch');
}
