 ...
 */
funcDecl
    : TO IDENT typeParamList? paramList? COMMA? PRODUCE annotatedType (DOT capabilityAnnotation? COLON NEWLINE block | COLON NEWLINE block | DOT)
    ;
 ...
 */
dataDecl
    : DEFINE article? TYPE_IDENT WITH fieldList DOT
    ;
 ...
 */
letStmt
    : LET IDENT BE lambdaExpr              # LetLambdaStmt
    | LET IDENT BE expr DOT                # LetExprStmt
    ;
...
comparisonExpr
    : additiveExpr (op=(LT | GT | LTE | GTE | NEQ) additiveExpr)?
    ;
...
multiplicativeExpr
    : unaryExpr (op=(STAR | SLASH) unaryExpr)*
    ;
