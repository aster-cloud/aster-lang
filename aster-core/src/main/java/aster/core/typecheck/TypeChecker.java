...
public final class TypeChecker {
  ...
  private void checkFunction(CoreModel.Func func, VisitorContext ctx) {
    ...
    // 同时存储函数声明的效果，用于后续效果推断
    var declaredEffect = func.effects.isEmpty()
      ? Optional.<String>empty()
      : Optional.of(func.effects.get(0));
    ...
    if (func.body != null) {
      ...
      // 检查效果兼容性
      var declaredEffectEnum = func.effects.isEmpty()
        ? EffectChecker.Effect.PURE
        : EffectChecker.Effect.fromString(func.effects.get(0));
      var inferredEffect = effectChecker.inferBlockEffect(func.body, ctx);

      effectChecker.checkEffectCompatibility(declaredEffectEnum, inferredEffect, Optional.ofNullable(func.origin));
