...
function formatStmt(s: Core.Statement, lvl: number): string {
  switch (s.kind) {
    case 'Let': return `val ${s.name} = ${formatExpr(s.expr)}`;
    case 'Set': return `${s.name} = ${formatExpr(s.expr)}`;
    case 'Return': return `return ${formatExpr(s.expr)}`;
    case 'If': {
    ...
    case 'Match': {
      const cases = s.cases.map(c => `${indent(lvl)}${formatPattern(c.pattern)} -> ${formatCaseBody(c.body, lvl)}`).join('\n');
...
${indent(lvl - 1)}}`;
    ...
    case 'Start': return `val ${s.name} = async { ${formatExpr(s.expr)} }`;
    case 'Wait': return `awaitAll(${s.names.join(', ')})`;
  }
}
...
function formatPattern(p: Core.Pattern): string {
  switch (p.kind) {
    ...
    case 'PatCtor': return `${p.typeName}(${p.names.join(', ')})`;
...
function formatExpr(e: Core.Expression): string {
  switch (e.kind) {
    ...
    case 'Ok': return `Ok(${formatExpr(e.expr)})`;
    case 'Err': return `Err(${formatExpr(e.expr)})`;
    case 'Some': return `Some(${formatExpr(e.expr)})`;
...
function formatType(t: Core.Type): string {
  switch (t.kind) {
    ...
    case 'Maybe': return `${formatType(t.type)}?`;
    case 'Option': return `Option<${formatType(t.type)}>`;
    case 'Result': return `Result<${formatType(t.ok)}, ${formatType(t.err)}>`;
    case 'List': return `List<${formatType(t.type)}>`;
    case 'Map': return `Map<${formatType(t.key)}, ${formatType(t.val)}>`;
  }
}
