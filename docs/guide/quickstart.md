...
```
# Parse
node dist/scripts/aster.js parse cnl/examples/greet.aster

# Core IR
node dist/scripts/aster.js core cnl/examples/greet.aster

# Emit Java sources
node dist/scripts/aster.js jvm cnl/examples/greet.aster --out build/jvm-src

# Emit classfiles (ASM)
node dist/scripts/aster.js class cnl/examples/login.aster --out build/jvm-classes
