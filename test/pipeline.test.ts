...
`;
      ...
      const enumDecl = result.core?.decls.find((d: Core.Declaration) => d.kind === 'Data');
...
`;
      ...
      const func = result.core?.decls.find((d: Core.Declaration) => d.kind === 'Func');
      assert.notEqual(func, undefined, '应该包含函数声明');
      assert.equal((func as any)?.effect, 'io', '效果应该是 io');
    });
  });
...
`;
    ...
    it('应该支持泛型数据类型', () => {
      const source = `
This module is test.result.

Define Result of T and E as one of Ok with value: T, Err with error: E.

To makeOk with val: Text, produce Result of Text and Text:
  Return Ok(val).
`;
      ...
      assert.equal(result.success, true, '泛型数据类型应该编译成功');
    });
...
To fetchDashboard with u: User, produce Text. It performs io:
