...
`);
    ...
    test('应该在多行参数中保留注解', () => {
      const module = parseSource(`
This module is test.parser.params.multiline_annotations.
...
`);
      ...
      assert.ok(queryParam.annotations && queryParam.annotations.length === 1);
      assert.equal(queryParam.annotations![0]!.name, 'NotEmpty');
    });
  });
  ...
  describe('注解错误处理', () => {
    test('应该在未知注解时抛出诊断', () => {
      assert.throws(
        () =>
          parseSource(`
This module is test.parser.annotations.error_unknown.

Define User with @Custom id: Text.
`),
        error => {
          assert.match(String(error), /未知注解/);
          return true;
        }
      );
    });

    test('应该在缺少必需注解参数时抛出诊断', () => {
      assert.throws(
        () =>
          parseSource(`
This module is test.parser.annotations.error_missing_param.

Define User with @Range(min: 0) age: Int.
`),
        error => {
          assert.match(String(error), /缺少必需参数 'max'/);
          return true;
        }
      );
    });

    test('应该在注解参数类型不匹配时抛出诊断', () => {
      assert.throws(
        () =>
          parseSource(`
This module is test.parser.annotations.error_type_mismatch.

Define User with @Pattern(regexp: 123) name: Text.
`),
        error => {
          assert.match(String(error), /需要字符串/);
          return true;
        }
      );
    });
