...
`);
  ...
  it('Data 字段注解应转换为普通对象', () => {
    const annotations = [
      { name: 'pii', params: new Map<string, unknown>([['category', 'email']]) },
    ] as const;
    ...
    const moduleAst = Node.Module('test.lowering.data.ann', [dataDecl]);
    const core = lowerAst(moduleAst);
    const data = core.decls[0] as Core.Data;
    const ann = data.fields[0]!.annotations![0]!;
    assert.equal(ann.name, 'pii');
    assert.equal((ann.params as Record<string, unknown>).category, 'email');
  });

  it('多注解字段与无注解字段应分别保留', () => {
    const fieldWithAnnotations: AstField = {
      name: 'id',
      type: Node.TypeName('Text'),
      annotations: [
        { name: 'primary', params: new Map<string, unknown>() },
        { name: 'pii', params: new Map<string, unknown>([['level', 'L1']]) },
      ],
      span: freshSpan(),
    };
    const fieldWithoutAnnotations: AstField = {
      name: 'age',
      type: Node.TypeName('Int'),
      span: freshSpan(),
    };
    const dataDecl = Node.Data('Profile', [fieldWithAnnotations, fieldWithoutAnnotations]);
    const moduleAst = Node.Module('test.lowering.data.multi', [dataDecl]);
    const core = lowerAst(moduleAst);
    const data = core.decls[0] as Core.Data;
    assert.equal(data.fields[0]!.annotations!.length, 2);
    assert.equal(data.fields[1]!.annotations!.length, 0);
  });
