...
import { loadEffectConfig, getIOPrefixes, getCPUPrefixes } from '../src/config/effect_config.js';
...
async function testFullConfig(): Promise<void> {
  ...
  // 重新加载模块以应用新配置
  delete require.cache[require.resolve('../src/config/effect_config.js')];
  const { getIOPrefixes: getIO } = await import(
    `../src/config/effect_config.js?t=${Date.now()}`
  );

  const prefixes = getIO();
  assertArrayContains(
    prefixes,
