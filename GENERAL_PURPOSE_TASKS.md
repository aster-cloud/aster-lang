...
- [ ] ASM coverage expansion and validation tests (switch, lambdas, try/catch).
  ...
  - [ ] Lambda/closure support: IR + JVM emission.
    - [ ] Add Core IR: function type and lambda expr nodes.
    - [ ] Capture analysis and lowering.
    - [ ] Runtime functional interfaces (Fn1/Fn2/...).
    - [ ] JVM emitter: closure class generation + apply method.
    - [ ] Golden + ASM fixtures (map with inline function, captured local).
    - [ ] CNL parser: lambda syntax (block form and/or `=>`) + lowering to Core.
    - [ ] Add golden tests for CNL lambdas once parsed (AST/Core paths).
- [ ] Truffle interpreter spike for Core IR (execution + profiling hooks).
  ...
  - [ ] Truffle: add Lambda node + execution support once IR lands.

## Docs & Examples

- [x] General purpose quickstart guide and project template walkthrough.
- [ ] End-to-end examples: CLI tool, REST service core, rules/policy engine.
- [ ] Language reference expansion (modules, effects, generics, stdlib spec).
  ...
  - [ ] Lambdas & closures: IR + emission design, syntax TBD (doc added).

## CI & Quality

- [ ] Performance budgets and microbench CI checks.
