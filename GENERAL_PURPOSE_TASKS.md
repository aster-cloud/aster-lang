...
- [x] Parametric polymorphism (generics) + improved inference in Core IR.
...
- [x] Truffle interpreter spike for Core IR (execution + profiling hooks).
  - [x] CLI to emit Core JSON and run via Truffle nodes (subset).
  - [x] Name lookup in conditions; Let/Return across blocks.
  - [x] Load Core JSON to full node tree (beyond subset) + profiling hooks.
...
- [x] End-to-end examples
  ...
  - [x] REST service core
  - [x] Rules/policy engine
- [x] Language reference expansion (modules, effects, generics, stdlib spec).
...
- [x] Golden tests for new AST/IR nodes and stdlib APIs.
  - [x] Add AST/Core golden for `cnl/examples/text_ops.cnl` and include in `scripts/golden.ts`.
- [x] Reproducible builds for CLI and docs (lockfile + pinned toolchain).
